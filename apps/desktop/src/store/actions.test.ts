...
describe("desktop recent workspaces", () => {
	beforeEach(() => {
		vi.unstubAllGlobals();
	});

	it("removes an inactive recent folder and persists the remaining state", async () => {
		const api = createDesktopApi();
		const {
			appStore,
		...
		removeRecentWorkspace("/workspace-b");
		...
		expect(switcherOpenStore.get()).toBe(true);
		expect(localStorage.setItem).toHaveBeenLastCalledWith(
			expect.any(String),
	...
	it("does not remove the active folder", async () => {
		const api = createDesktopApi();
		const { appStore, removeRecentWorkspace, workspaceStore } =
			await loadStoreActions(api);
		...
		removeRecentWorkspace("/workspace-a");

		expect(workspaceStore.get().recentWorkspaces).toEqual([
			"/workspace-a",
			"/workspace-b",
		]);
	});
